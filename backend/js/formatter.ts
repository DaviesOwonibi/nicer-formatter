...
import { Expr, ObjectDeclaration, Program, Stmt } from "../../frontend/js/ast.ts";
    ...
    if (rules.hasOwnProperty("semicolons") && rules["semicolons"] == true) {
      switch (node.kind) {
        case "ObjectDeclaration":
          ...
          const propertiesFormatted = props.map(prop => `${objectTabs}${prop.key.symbol}: ${formatNode(prop.value)}`).join(",\n");
          ...
          forParam.forEach((parameter: Expr) => {
            if (forCount != 0) {
              if (forCount == 1) {
                ...
                if (parameter) {
                  forParams.push(" " + formatNode(parameter).replace(";", ""));
                } else {
                  forParams.push("");
                }
              }
